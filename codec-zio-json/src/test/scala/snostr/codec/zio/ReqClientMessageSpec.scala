...
class ReqClientMessageSpec extends AnyFlatSpec with Matchers {
  it should "encode/decode REQ" in {
    ...
    JsonDecoder[ReqClientMessage].decodeJson("""["REQ","subid"]""") should be(Left("(invalid REQ message)"))
    JsonDecoder[ReqClientMessage].decodeJson("""["REQ","subid", ""]""") should be(Left("([0](expected '{' got '\"'))"))

    val Right(msg) = JsonDecoder[ReqClientMessage].decodeJson("""["REQ","subid", {}, {}]""")
    ...
    val decoded = JsonDecoder[ReqClientMessage].decodeJson(json)
