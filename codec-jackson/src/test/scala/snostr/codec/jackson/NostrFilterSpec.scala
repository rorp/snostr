...
class NostrFilterSpec extends AnyFlatSpec with Matchers {
  it should "serialize/deserialize" in {
    ...
    val decoded1 = serialization.read[NostrFilter](sample1)
    ...
    val encoded = serialization.write(filter)
    encoded shouldNot contain("null")
    val decoded = serialization.read[NostrFilter](encoded)

    decoded should be(filter)
  }
}
