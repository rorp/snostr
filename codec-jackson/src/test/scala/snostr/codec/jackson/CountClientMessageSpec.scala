...
class CountClientMessageSpec extends AnyFlatSpec with Matchers {
  it should "serialize/deserialize COUNT" in {
    assertThrows[MismatchedInputException](serialization.read[CountClientMessage](""))
    assertThrows[MappingException](serialization.read[CountClientMessage]("[]"))
    assertThrows[MappingException](serialization.read[CountClientMessage]("""["COUNT"]"""))
    assertThrows[MappingException](serialization.read[CountClientMessage]("""["COUNT","subid"]"""))
    assertThrows[MappingException](serialization.read[CountClientMessage]("""["COUNT","subid", ""]"""))

    val msg = serialization.read[CountClientMessage]("""["COUNT","subid", {}, {}]""")
    ...
    val decoded = serialization.read[CountClientMessage](json)
